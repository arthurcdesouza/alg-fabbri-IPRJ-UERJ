...
typedef struct lista *lista_ptr; typedef struct no *no_ptr;
typedef struct lista { no_ptr no; int peso; struct lista *next; } lista;
typedef struct no { char nome[32]; lista *conexoes; } no;

void insere(lista **lst, no_ptr no, int peso) {
   lista *node = (lista *) malloc(sizeof(lista));
   node->peso = peso; node->no = no; node->next = *lst; *lst = node;
}
void liga(no_ptr a, no_ptr b, int p) { insere(&a->conexoes, b, p); insere(&b->conexoes, a, p); }
...
int main() { // monta um grafo e testa
  ...
  for (int i = 0; i < 5; ++i) { strcpy(nos[i].nome, nomes[i]); nos[i].conexoes = NULL; }
  
  liga(nos+0, nos+1, 50); liga(nos+0, nos+3, 65); liga(nos+1, nos+3, 100); liga(nos+1, nos+4, 75);
  liga(nos+2, nos+3, 15); liga(nos+2, nos+4, 120); liga(nos+3, nos+4, 140);

/*  deleta_no_grafo(nos+3); // deleta Rio*/
  
  renomeia_friburgo(nos+0);
  no_ptr nos_componente[MAX_NUM_NOS]; int n_nos = 0, peso_total;
  if ((peso_total = componente_conexo(nos+4, 100, nos_componente, &n_nos)) == -1 ) return 1;
